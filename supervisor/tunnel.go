...
import (
	...
	"github.com/khulnasoft/tunnellink/edgediscovery/allregions"
	...
	quicpogs "github.com/khulnasoft/tunnellink/quic"
	v3 "github.com/khulnasoft/tunnellink/quic/v3"
	"github.com/khulnasoft/tunnellink/retry"
...
func (e *EdgeTunnelServer) Serve(ctx context.Context, connIndex uint8, protocolFallback *protocolFallback, connectedSignal *signal.Signal) error {
	...
	logger := e.config.Log.With().
		Int(management.EventTypeKey, int(management.Tunnellink)).
...
func selectNextProtocol(
	...
	if protocolBackoff.ReachedMaxRetries() || (hasFallback && isQuicBroken) {
		if isQuicBroken {
			connLog.Warn().Msg("If this log occurs persistently, and tunnellink is unable to connect to " +
				...
				"unless your tunnellink can connect with Khulnasoft Network with `quic`.")
		}
