...
import (
	...
	"github.com/khulnasoft/tunnellink/hello"
	"github.com/khulnasoft/tunnellink/ipaccess"
	"github.com/khulnasoft/tunnellink/management"
	"github.com/khulnasoft/tunnellink/socks"
...
type OriginService interface {
	String() string
	// Start the origin service if it's managed by tunnellink, e.g. proxy servers or Hello World.
	// If it's not managed by tunnellink, this is a no-op because the user is responsible for
	// starting the origin service.
	// Implementor of services managed by tunnellink should terminate the service if shutdownC is closed
...
// Users only use this for testing and experimenting with tunnellink.
...
// default status code (503) that is returned for requests to tunnellink that don't have any ingress rules setup
