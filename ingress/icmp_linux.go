...
func testPermission(listenIP netip.Addr, zone string, logger *zerolog.Logger) error {
	...
	if listenIP.Is4() {
		if err := checkInPingGroup(); err != nil {
			logger.Warn().Err(err).Msgf("The user running tunnellink process has a GID (group ID) that is not within ping_group_range. You might need to add that user to a group within that range, or instead update the range to encompass a group the user is already in by modifying %s. Otherwise tunnellink will not be able to ping this network", pingGroupPath)
			return err
		}
	}
	...
	// This conn is only to test if tunnellink has permission to open this type of socket
	conn.Close()
	return nil
}
