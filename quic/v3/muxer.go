...
func (c *datagramConn) Serve(ctx context.Context) error {
	...
	for {
		// We make sure to monitor the context of tunnellink and the underlying connection to return if any errors occur.
		var datagram []byte
		select {
		// Monitor the context of tunnellink
		case <-ctx.Done():
			return ctx.Err()
		...
		go func() {
			...
			case UDPSessionRegistrationType:
				...
				// We bind the new session to the quic connection context instead of tunnellink context to allow for the
				// quic connection to close and close only the sessions bound to it. Closing of tunnellink will also
			...
			case UDPSessionRegistrationResponseType:
				// tunnellink should never expect to receive UDP session responses as it will not initiate new
