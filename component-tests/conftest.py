        ...
        def _component_tests_config(additional_config={}, cfd_mode=CfdModes.NAMED, run_proxy_dns=True, provide_ingress=True):
            ...
            if cfd_mode is CfdModes.NAMED:
                return NamedTunnelConfig(additional_config=additional_config,
                                         tunnellink_binary=config['tunnellink_binary'],
                                         tunnel=config['tunnel'],
                                         credentials_file=config['credentials_file'],
                                         ingress=ingress,
                                         hostname=hostname)
            elif cfd_mode is CfdModes.PROXY_DNS:
                return ProxyDnsConfig(tunnellink_binary=config['tunnellink_binary'])
            elif cfd_mode is CfdModes.QUICK:
                return QuickTunnelConfig(additional_config=additional_config, tunnellink_binary=config['tunnellink_binary'])
            else:
                raise Exception(f"Unknown tunnellink mode {cfd_mode}")

        return _component_tests_config


# This fixture is automatically called before each tests to make sure the previous tunnellink has been shutdown
@pytest.fixture(autouse=True)
def wait_previous_tunnellink():
    sleep(BACKOFF_SECS)
