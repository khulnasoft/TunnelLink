...
class TestReconnect:
    ...
    def test_named_reconnect(self, tmp_path, component_tests_config, protocol):
        ...
        with start_tunnellink(tmp_path, config, cfd_pre_args=["tunnel", "--ha-connections", "1"], new_process=True, allow_input=True, capture_output=False) as tunnellink:
            # Repeat the test multiple times because some issues only occur after multiple reconnects
            self.assert_reconnect(config, tunnellink, 5)

    def send_reconnect(self, tunnellink, secs):
        ...
        tunnellink.stdin.write(f"reconnect {secs}s\n".encode())
        tunnellink.stdin.flush()

    def assert_reconnect(self, config, tunnellink, repeat):
        wait_tunnel_ready(tunnel_url=config.get_url(),
                          require_min_connections=self.default_ha_conns)
        for _ in range(repeat):
            for _ in range(self.default_ha_conns):
                self.send_reconnect(tunnellink, self.default_reconnect_secs)
