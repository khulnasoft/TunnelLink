...
from util import start_tunnellink, wait_tunnel_ready, send_requests

# Rolling logger rotate log files after 1 MB
rotate_after_size = 1000 * 1000
default_log_file = "tunnellink.log"
expect_message = "Starting Hello"


def assert_log_to_terminal(tunnellink):
    for _ in range(0, MAX_LOG_LINES):
        line = tunnellink.stderr.readline()
        if not line:
            break
...
class TestLogging:
    def test_logging_to_terminal(self, tmp_path, component_tests_config):
        ...
        with start_tunnellink(tmp_path, config, cfd_pre_args=["tunnel", "--ha-connections", "1"], new_process=True) as tunnellink:
            wait_tunnel_ready(tunnel_url=config.get_url())
            assert_log_to_terminal(tunnellink)
    ...
    def test_logging_to_file(self, tmp_path, component_tests_config):
        ...
        with start_tunnellink(tmp_path, config, cfd_pre_args=["tunnel", "--ha-connections", "1"], new_process=True, capture_output=False):
