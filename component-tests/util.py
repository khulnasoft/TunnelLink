...
def start_tunnellink(directory, config, cfd_args=["run"], cfd_pre_args=["tunnel"], new_process=False,
    ...
    cmd = tunnellink_cmd(config, config_path, cfd_args, cfd_pre_args, root)

    if new_process:
        return run_tunnellink_background(cmd, allow_input, capture_output)
...
def tunnellink_cmd(config, config_path, cfd_args, cfd_pre_args, root):
    cmd = []
    if root:
        cmd += ["sudo"]
    cmd += [config.tunnellink_binary]
    cmd += cfd_pre_args
...
@contextmanager
def run_tunnellink_background(cmd, allow_input, capture_output):
    ...
    try:
        ...
        if cfd:
            cfd.terminate()
            if capture_output:
                LOGGER.info(f"tunnellink log: {cfd.stderr.read()}")
...
def _log_tunnellink_logs(cfd_logs):
    log_file = cfd_logs
    ...
    with open(log_file, "r") as f:
        LOGGER.warning("Tunnellink Tunnel was not ready:")
...
def check_tunnel_not_connected():
    ...
    # tunnellink might already terminate
...
def get_tunnel_connector_id():
    ...
    # tunnellink might already terminated
    except requests.exceptions.ConnectionError as e:
        LOGGER.warning(f"Failed to connect to {url}, error: {e}")
