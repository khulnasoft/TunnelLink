...
from util import select_platform, start_tunnellink, wait_tunnel_ready, write_config


def default_config_dir():
    return os.path.join(Path.home(), ".tunnellink")
...
class TestServiceMode:
    ...
    def test_launchd_service_log_to_file(self, tmp_path, component_tests_config):
        ...
        additional_config = {
            # On Darwin tunnellink service defaults to run classic tunnel command
            "hello-world": True,
    ...
    @pytest.mark.skipif(os.path.exists("/etc/tunnellink/config.yml"),
    ...
    def test_sysv_service_with_token(self, tmp_path, component_tests_config):
        ...
        subprocess.run(["sudo", "cp", config_path, "/etc/tunnellink/config.yml"], check=True)
    ...
    def sysv_service_scenario(self, config, tmp_path, extra_assertions=None, use_token=False):
        ...
        # Service install copies config file to /etc/tunnellink/config.yml
        subprocess.run(["sudo", "rm", "/etc/tunnellink/config.yml"])
