...
type BuildInfo struct {
	...
	TunnellinkVersion string `json:"tunnellink_version"`
...
func GetBuildInfo(buildType, version string) *BuildInfo {
	return &BuildInfo{
		...
		BuildType:         buildType,
		TunnellinkVersion: version,
		Checksum:          currentBinaryChecksum(),
	}
}

func (bi *BuildInfo) Log(log *zerolog.Logger) {
	log.Info().Msgf("Version %s (Checksum %s)", bi.TunnellinkVersion, bi.Checksum)
	if bi.BuildType != "" {
...
func (bi *BuildInfo) UserAgent() string {
	return fmt.Sprintf("tunnellink/%s", bi.TunnellinkVersion)
}
