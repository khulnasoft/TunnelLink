...
const (
	windowsServiceName        = "Tunnellink"
	windowsServiceDescription = "Tunnellink agent"
...
func runApp(app *cli.App, graceShutdownC chan struct{}) {
	app.Commands = append(app.Commands, &cli.Command{
		Name:  "service",
		Usage: "Manages the tunnellink Windows service",
		Subcommands: []*cli.Command{
			{
				Name:   "install",
				Usage:  "Install tunnellink as a Windows service",
	...
	// like running tunnellink in a GCP startup script, for which
...
func (s *windowsService) Execute(serviceArgs []string, r <-chan svc.ChangeRequest, statusChan chan<- svc.Status) (ssec bool, errno uint32) {
	...
	for {
		...
		case c := <-r:
			...
			case svc.Stop, svc.Shutdown:
				if s.graceShutdownC != nil {
					...
					elog.Info(1, "tunnellink starting graceful shutdown")
				...
				elog.Info(1, "tunnellink terminating immediately")
				statusChan <- svc.Status{State: svc.StopPending}
				return false, 0
			default:
		...
		case err := <-errC:
			if err != nil {
				elog.Error(1, fmt.Sprintf("tunnellink terminated with error %v", err))
				ssec = true
				errno = 1
			} else {
				elog.Info(1, "tunnellink terminated without error")
				errno = 0
			}
			return
		}
...
func installWindowsService(c *cli.Context) error {
	...
	zeroLogger.Info().Msg("Installing tunnellink Windows service")
	...
	defer s.Close()
	log.Info().Msg("tunnellink agent service is installed")
	...
	err = s.Start()
	if err == nil {
		log.Info().Msg("Agent service for tunnellink installed successfully")
	}
	return err
}
...
func uninstallWindowsService(c *cli.Context) error {
	...
	log.Info().Msg("Uninstalling tunnellink agent service")
	m, err := mgr.Connect()
	if err != nil {
		return errors.Wrap(err, "Cannot establish a connection to the service control manager")
	}
	...
	if status, err := s.Query(); err == nil && status.State == svc.Running {
		log.Info().Msg("Stopping tunnellink agent service")
		if _, err := s.Control(svc.Stop); err != nil {
			log.Info().Err(err).Msg("Failed to stop tunnellink agent service, you may need to stop it manually to complete uninstall.")
		}
	}
	...
	log.Info().Msg("Agent service for tunnellink was uninstalled successfully")
