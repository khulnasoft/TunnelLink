...
const (
	...
	// stop the service
	// rename tunnellink.exe to tunnellink.exe.old
	// rename tunnellink.exe.new to tunnellink.exe
	// delete tunnellink.exe.old
	// start the service
	// exit with code 0 if we've reached this point indicating success.
	windowsUpdateCommandTemplate = `sc stop tunnellink >nul 2>&1
del "{{.OldPath}}"
rename "{{.TargetPath}}" {{.OldName}}
rename "{{.NewPath}}" {{.BinaryName}}
sc start tunnellink >nul 2>&1
exit /b 0`
...
// target path is where the file should be replace. Normally this the running tunnellink's path
// userMessage is a possible message to convey back to the user after having checked in with the Updater Service
// isCompressed tells whether the asset to update tunnellink is compressed or not
...
func (v *WorkersVersion) Apply() error {
	...
	// the easiest way to do this is with a batch file (or with a DLL, but that gets ugly for a cross compiled binary like tunnellink)
