...
const (
	...
	noUpdateInShellMessage        = "tunnellink will not automatically update when run from the shell. To enable auto-updates, run tunnellink as a service: https://developers.khulnasoft.com/khulnasoft-one/connections/connect-apps/run-tunnel/as-a-service/"
	noUpdateOnWindowsMessage      = "tunnellink will not automatically update on Windows systems."
	noUpdateManagedPackageMessage = "tunnellink will not automatically update if installed by a package manager."
...
func (u *statusSuccess) Error() string {
	return fmt.Sprintf("tunnellink has been updated to version %s", u.newVersion)
}
...
func (e *statusErr) Error() string {
	return fmt.Sprintf("failed to update tunnellink: %v", e.err)
}
...
func CheckForUpdate(options updateOptions) (CheckResult, error) {
	...
	s := NewWorkersService(buildInfo.TunnellinkVersion, url, cfdPath, Options{IsBeta: options.isBeta,
...
func Update(c *cli.Context) error {
	...
	if wasInstalledFromPackageManager() {
		...
		log.Error().Msg(fmt.Sprintf("tunnellink was installed by %s. Please update using the same method.", packageManagerName))
		return nil
	}

	isBeta := c.Bool("beta")
	if isBeta {
		log.Info().Msg("tunnellink is set to update to the latest beta version")
	}

	isStaging := c.Bool("staging")
	if isStaging {
		log.Info().Msg("tunnellink is set to update from staging")
	}

	isForced := c.Bool("force")
	if isForced {
		log.Info().Msg("tunnellink is set to upgrade to the latest publish version regardless of the current version")
	}
	...
	if updateOutcome.noUpdate() {
		log.Info().Str(LogFieldVersion, updateOutcome.Version).Msg("tunnellink is up to date")
		return nil
	}
...
func loggedUpdate(log *zerolog.Logger, options updateOptions) UpdateOutcome {
	...
	if updateOutcome.Updated {
		log.Info().Str(LogFieldVersion, updateOutcome.Version).Msg("tunnellink has been updated")
	}
	if updateOutcome.Error != nil {
...
// Run will perodically check for tunnellink updates, download them, and then restart the current tunnellink process
...
func (a *AutoUpdater) Run(ctx context.Context) error {
	...
	for {
		...
		if updateOutcome.Updated {
			buildInfo.TunnellinkVersion = updateOutcome.Version
			if IsSysV() {
