...
import (
	...
	"github.com/khulnasoft/tunnellink/credentials"
	"github.com/khulnasoft/tunnellink/diagnostic"
	"github.com/khulnasoft/tunnellink/edgediscovery"
	"github.com/khulnasoft/tunnellink/features"
	"github.com/khulnasoft/tunnellink/ingress"
	"github.com/khulnasoft/tunnellink/logger"
	"github.com/khulnasoft/tunnellink/management"
	"github.com/khulnasoft/tunnellink/metrics"
	"github.com/khulnasoft/tunnellink/orchestration"
	"github.com/khulnasoft/tunnellink/signal"
	"github.com/khulnasoft/tunnellink/supervisor"
	"github.com/khulnasoft/tunnellink/tlsconfig"
	"github.com/khulnasoft/tunnellink/tunneldns"
	"github.com/khulnasoft/tunnellink/tunnelstate"
	"github.com/khulnasoft/tunnellink/validation"
)
...
const (
	...
	// sshPortFlag is the port on localhost the tunnellink ssh server will run on
	...
	// uiFlag is to enable launching tunnellink in interactive UI mode
	uiFlag = "ui"
	...
	tunnelCmdErrorMessage = `You did not specify any valid additional argument to the tunnellink tunnel command.

If you are trying to run a Quick Tunnel then you need to explicitly pass the --url flag.
Eg. tunnellink tunnel --url localhost:8080/.
...
Alternatively, you can manage your Tunnels via the command line. Begin by obtaining a certificate to be able to do so:
	...
	$ tunnellink tunnel login

With your certificate installed you can then get started with Tunnels:

	$ tunnellink tunnel create my-first-tunnel
	$ tunnellink tunnel route dns my-first-tunnel my-first-tunnel.mydomain.com
	$ tunnellink tunnel run --hello-world my-first-tunnel

You can now access my-first-tunnel.mydomain.com and be served an example page by your local tunnellink process.
...
func TunnelCommand(c *cli.Context) error {
	...
	if ref := config.GetConfiguration().TunnelID; ref != "" {
		return fmt.Errorf("Use `tunnellink tunnel run` to start tunnel %s", ref)
	}
	...
	if c.IsSet("proxy-dns") {
		...
		// not run as part of tunnellink tunnel.
...
func StartServer(
	...
	go func() {
		...
		diagnosticHandler := diagnostic.NewDiagnosticHandler(log, 0, diagnostic.NewSystemCollectorImpl(buildInfo.TunnellinkVersion))
...
func tunnelFlags(shouldHide bool) []cli.Flag {
	flags := configureTunnellinkFlags(shouldHide)
	...
	flags = append(flags, []cli.Flag{
		...
		altsrc.NewDurationFlag(&cli.DurationFlag{
			Name:    "grace-period",
			Usage:   "When tunnellink receives SIGINT/SIGTERM it will stop accepting new requests, wait for in-progress requests to terminate, then shutdown. Waiting for in-progress requests will timeout after this grace period, or when a second SIGTERM/SIGINT is received.",
		...
		altsrc.NewIntFlag(&cli.IntFlag{
			Name:    "max-fetch-size",
			Usage:   `The maximum number of results that tunnellink can fetch from Khulnasoft API for any listing operations needed`,
...
func configureTunnellinkFlags(shouldHide bool) []cli.Flag {
	return []cli.Flag{
		&cli.StringFlag{
			Name:   "config",
		...
		altsrc.NewStringFlag(&cli.StringFlag{
			...
			Usage:   "Path to the certificate generated for your origin when you run tunnellink login.",
		...
		altsrc.NewStringFlag(&cli.StringFlag{
			...
			Usage: fmt.Sprintf(
				`Listen address for metrics reporting. If no address is passed tunnellink will try to bind to %v.
If all are unavailable, a random port will be used. Note that when running tunnellink from an virtual
...
func sshFlags(shouldHide bool) []cli.Flag {
	return []cli.Flag{
		altsrc.NewStringFlag(&cli.StringFlag{
			Name:    sshPortFlag,
			Usage:   "Localhost port that tunnellink SSH server will run on",
			Value:   "2222",
