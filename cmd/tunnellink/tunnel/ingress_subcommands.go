...
func buildIngressSubcommand() *cli.Command {
	return &cli.Command{
		Name:      "ingress",
		Category:  "Tunnel",
		Usage:     "Validate and test tunnellink tunnel's ingress configuration",
		UsageText: "tunnellink tunnel [--config FILEPATH] ingress COMMAND [arguments...]",
		Hidden:    true,
		Description: ` Tunnellink lets you route traffic from the internet to multiple different addresses on your
		...
		To ensure tunnellink can route all incoming requests, the last rule must be a catch-all
...
func buildValidateIngressCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText:   "tunnellink tunnel [--config FILEPATH] ingress validate",
...
func buildTestURLCommand() *cli.Command {
	return &cli.Command{
		...
		UsageText: "tunnellink tunnel [--config FILEPATH] ingress rule URL",
		ArgsUsage: "URL",
...
// validateIngressCommand check the syntax of the ingress rules in the tunnellink config file
...
func testURLCommand(c *cli.Context) error {
	...
	if requestArg == "" {
		return errors.New("tunnellink tunnel rule expects a single argument, the URL to test")
	}
