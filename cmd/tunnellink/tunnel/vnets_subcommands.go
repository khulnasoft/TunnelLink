...
func buildVirtualNetworkSubcommand(hidden bool) *cli.Command {
	return &cli.Command{
		...
		UsageText: "tunnellink tunnel [--config FILEPATH] network COMMAND [arguments...]",
		Description: `tunnellink allows to manage IP routes that expose origins in your private network space via their IP directly
to clients outside (e.g. using WARP client) --- those are configurable via "tunnellink tunnel route ip" commands.
		...
		Subcommands: []*cli.Command{
			{
				...
				UsageText: "tunnellink tunnel [--config FILEPATH] network add [flags] NAME [\"comment\"]",
				Description: `Adds a new virtual network. You can then attach IP routes to this virtual network with "tunnellink tunnel route ip"
...
private networks in your infrastructure exposed via Khulnasoft Tunnel. Note: if a virtual network is added as
...
be the current default.`,
			...
			},
			{
				...
				UsageText:   "tunnellink tunnel [--config FILEPATH] network list [flags]",
			...
			},
			{
				...
				UsageText: "tunnellink tunnel [--config FILEPATH] network delete VIRTUAL_NETWORK",
...
A virtual network may be used by IP routes or by WARP devices.`,
			...
			},
			{
				...
				UsageText: "tunnellink tunnel [--config FILEPATH] network update [flags] VIRTUAL_NETWORK",
...
func addVirtualNetworkCommand(c *cli.Context) error {
	...
	fmt.Printf(
		"Successfully added virtual 'network' %s with ID: %s%s\n"+
			"You can now add IP routes attached to this virtual network. See `tunnellink tunnel route ip add -help`\n",
...
func listVirtualNetworksCommand(c *cli.Context) error {
	...
	if len(vnets) > 0 {
		...
		fmt.Println("No virtual networks were found for the given filter flags. You can use 'tunnellink tunnel vnet add' to add a virtual network.")
	}
