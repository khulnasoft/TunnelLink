...
func buildRouteIPSubcommand() *cli.Command {
	return &cli.Command{
		...
		UsageText: "tunnellink tunnel [--config FILEPATH] route COMMAND [arguments...]",
		Description: `tunnellink can provision routes for any IP space in your corporate network. Users enrolled in
...
See "tunnellink tunnel vnet --help" for more information.`,
		Subcommands: []*cli.Command{
			{
				Name:      "add",
				Action:    cliutil.ConfiguredAction(addRouteCommand),
				Usage:     "Add a new network to the routing table reachable via a Tunnel",
				UsageText: "tunnellink tunnel [--config FILEPATH] route ip add [flags] [CIDR] [TUNNEL] [COMMENT?]",
...
"tunnellink tunnel vnet --help)". In those cases, you then have to tell
which virtual network's routing table you want to add the route to with:
"tunnellink tunnel route ip add --vnet [ID/name] [CIDR] [TUNNEL]".`,
			...
			},
			{
				...
				UsageText:   "tunnellink tunnel [--config FILEPATH] route ip show [flags]",
			...
			},
			{
				...
				UsageText: "tunnellink tunnel [--config FILEPATH] route ip delete [flags] [Route ID or CIDR]",
...
will no longer be reachable.`,
			...
			},
			{
				...
				UsageText: "tunnellink tunnel [--config FILEPATH] route ip get [flags] [IP]",
...
func showRoutesCommand(c *cli.Context) error {
	...
	if len(routes) > 0 {
		...
		fmt.Println("No routes were found for the given filter flags. You can use 'tunnellink tunnel route ip add' to add a route.")
	}
