...
import (
	...
	"github.com/khulnasoft/tunnellink/sshgen"
...
{{- if .ShortLivedCerts}}
Match host {{.Hostname}} exec "{{.Tunnellink}} access ssh-gen --hostname %h"
  ProxyCommand {{.Tunnellink}} access ssh --hostname %h
  IdentityFile ~/.tunnellink/%h-cf_key
  CertificateFile ~/.tunnellink/%h-cf_key-cert.pub
{{- else}}
Host {{.Hostname}}
  ProxyCommand {{.Tunnellink}} access ssh --hostname %h
{{end}}
...
func Init(shutdown chan struct{}, version string) {
	shutdownC = shutdown
	userAgent = fmt.Sprintf("tunnellink/%s", version)
}
			...
			Subcommands: []*cli.Command{
				{
					...
					administrator. tunnellink stores the token in local storage.`,
					Flags: []cli.Flag{
						&cli.BoolFlag{
							Name:    loginQuietFlag,
							Aliases: []string{"q"},
...
func sshConfig(c *cli.Context) error {
	...
	return t.Execute(os.Stdout, config{Home: os.Getenv("HOME"), ShortLivedCerts: genCertBool, Hostname: hostname, Tunnellink: tunnellinkPath()})
}
...
// tunnellinkPath pulls the full path of tunnellink on disk
func tunnellinkPath() string {
	...
	for _, p := range strings.Split(os.Getenv("PATH"), ":") {
		path := fmt.Sprintf("%s/%s", p, "tunnellink")
		if isFileThere(path) {
			return path
		}
	}
...
func isTokenValid(options *carrier.StartOptions, log *zerolog.Logger) (bool, error) {
	...
	query := req.URL.Query()
	query.Set("tunnellink_token_check", "true")
