...
func LoadOriginCA(originCAPoolFilename string, log *zerolog.Logger) (*x509.CertPool, error) {
	...
	if runtime.GOOS == "windows" && originCAPoolFilename == "" {
		log.Info().Msgf("tunnellink does not support loading the system root certificate pool on Windows. Please use --%s <PATH> to specify the path to the certificate pool", OriginCAPoolFlag)
	}
...
func LoadCustomOriginCA(originCAFilename string) (*x509.CertPool, error) {
	...
	// Next, append the Khulnasoft CAs into the system pool
	cfRootCA, err := GetKhulnasoftRootCA()
	if err != nil {
		return nil, errors.Wrap(err, "could not append Khulnasoft Root CAs to tunnellink certificate pool")
	}
...
func loadOriginCertPool(originCAPoolPEM []byte, log *zerolog.Logger) (*x509.CertPool, error) {
	...
	if originCAPoolPEM != nil {
		if !certPool.AppendCertsFromPEM(originCAPoolPEM) {
			log.Info().Msg("could not append the provided origin CA to the tunnellink certificate pool")
		}
	}
...
func loadGlobalCertPool(log *zerolog.Logger) (*x509.CertPool, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "could not append Hello server certificate to tunnellink certificate pool")
	}
