...
TUNNELLINK_REPO = os.environ.get("GITHUB_REPO", "khulnasoft/tunnellink")
...
def upload_asset(release, filepath, filename, release_version, kv_account_id, namespace_id, kv_api_token):
    ...
    if binary_path.endswith("tgz"):
        ...
        binary_path = os.path.join(os.getcwd(), 'cfd', 'tunnellink')
...
def assert_asset_version(binary_path, release_version):
    ...
    if ext == '.exe' or ext == '':
        ...
        tar.extractall("tmp")
        tar.close()
        binary_path = os.path.join(os.getcwd(), 'tmp', 'tunnellink')
        binary_version = get_binary_version(binary_path)
    else:
        return
...
def main():
    ...
    try:
        ...
        if args.dry_run:
            ...
            repo = client.get_repo(TUNNELLINK_REPO)
