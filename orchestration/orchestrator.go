...
import (
	...
	"github.com/khulnasoft/tunnellink/proxy"
...
type Orchestrator struct {
	...
	proxy atomic.Value
	// Set of internal ingress rules defined at tunnellink startup (separate from user-defined ingress rules)
	internalRules []ingress.Rule
	// tunnellink Configuration
	config *Config
	tags   []pogs.Tag
	log    *zerolog.Logger
...
func (o *Orchestrator) updateIngress(ingressRules ingress.Ingress, warpRouting ingress.WarpRoutingConfig) error {
	select {
	case <-o.shutdownC:
		return fmt.Errorf("tunnellink already shutdown")
	default:
	}
