...
import (
	...
	cfdquic "github.com/khulnasoft/tunnellink/quic"
	...
	rpcquic "github.com/khulnasoft/tunnellink/tunnelrpc/quic"
)
...
func (q *quicConnection) Serve(ctx context.Context) error {
	...
	// In the future, if tunnellink can autonomously push traffic to the edge, we have to make sure the control
...
func (q *quicConnection) runStream(quicStream quic.Stream) {
	...
	// we are going to fuse readers/writers from stream <- tunnellink -> origin, and we want to guarantee that
	...
	ss := rpcquic.NewTunnellinkServer(q.handleDataStream, q.datagramHandler, q, q.rpcTimeout)
...
func (s *streamReadWriteAcker) AckConnection(tracePropagation string) error {
	...
	if tracePropagation != "" {
		metadata = append(metadata, pogs.Metadata{
			Key: tracing.CanonicalTunnellinkTracingHeader,
