...
func (h *HTTPResponseReadWriteAcker) AckConnection(tracePropagation string) error {
	...
	if tracePropagation != "" {
		resp.Header.Add(tracing.CanonicalTunnellinkTracingHeader, tracePropagation)
	}

	return h.w.WriteRespHeaders(resp.StatusCode, resp.Header)
}

// localProxyConnection emulates an incoming connection to tunnellink as a net.Conn.
