...
func TestIsControlResponseHeader(t *testing.T) {
	controlResponseHeaders := []string{
		// Anything that begins with cf-int- or cf-tunnellink-
		"cf-int-sample-header",
		"cf-tunnellink-sample-header",
...
func TestIsNotControlResponseHeader(t *testing.T) {
	notControlResponseHeaders := []string{
		...
		"upgrade",
		"connection",
		"cf-whatever", // On the response path, we only want to filter cf-int- and cf-tunnellink-
	}
