...
import (
	...
	tunnelpogs "github.com/khulnasoft/tunnellink/tunnelrpc/pogs"
)

// note: these constants are exported so we can reuse them in the edge-side code
const (
	InternalUpgradeHeader     = "Cf-Tunnellink-Proxy-Connection-Upgrade"
	InternalTCPProxySrcHeader = "Cf-Tunnellink-Proxy-Src"
...
// HTTP2Connection represents a net.Conn that uses HTTP2 frames to proxy traffic from the edge to tunnellink on the
// origin.
...
func (c *HTTP2Connection) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case TypeTCP:
		...
		if err != nil {
			requestErr = fmt.Errorf(`tunnellink received a warp-routing request with an empty host value: %w`, err)
			break
		}
...
func (rp *http2RespWriter) WriteRespHeaders(status int, header http.Header) error {
	...
	for name, values := range header {
		...
		if h2name == tracing.IntTunnellinkTracingHeader {
			// Add cf-int-tunnellink-tracing header outside of serialized userHeaders
			dest[tracing.CanonicalTunnellinkTracingHeader] = values
			continue
		}
