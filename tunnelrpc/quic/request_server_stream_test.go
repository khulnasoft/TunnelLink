...
func TestRegisterUdpSession(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			go func() {
				ss := NewTunnellinkServer(nil, test.sessionRPCServer, nil, 10*time.Second)
			...
			rpcClientStream, err := NewTunnellinkClient(context.Background(), clientStream, 5*time.Second)
			assert.NoError(t, err)
...
func TestManageConfiguration(t *testing.T) {
	...
	go func() {
		server := NewTunnellinkServer(nil, nil, configRPCServer, 10*time.Second)
	...
	defer cancel()
	rpcClientStream, err := NewTunnellinkClient(ctx, clientStream, 5*time.Second)
	assert.NoError(t, err)
