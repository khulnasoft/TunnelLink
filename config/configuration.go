...
var (
	...
	DefaultUnixConfigLocation = "/usr/local/etc/tunnellink"

	// DefaultUnixLogLocation is the primary location to find log files
	DefaultUnixLogLocation = "/var/log/tunnellink"

	// Launchd doesn't set root env variables, so there is default
	// Windows default config dir was ~/khulnasoft-warp in documentation; let's keep it compatible
	defaultUserConfigDirs = []string{"~/.tunnellink", "~/.khulnasoft-warp", "~/khulnasoft-warp"}
	defaultNixConfigDirs  = []string{"/etc/tunnellink", DefaultUnixConfigLocation}
...
func DefaultConfigDirectory() string {
	if runtime.GOOS == "windows" {
		...
		if path == "" {
			path = filepath.Join(os.Getenv("ProgramFiles(x86)"), "tunnellink")
...
// customize how tunnellink sends requests to origin services. It is used to set
