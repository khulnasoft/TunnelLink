...
// Example: assigning LogEventType.Tunnellink to a zerolog event will allow the client to filter for only
// the Tunnellink-related events.
type LogEventType int8

const (
	// Tunnellink events are signficant to tunnellink operations like connection state changes.
	// Tunnellink is also the default event type for any events that haven't been separated into a proper event type.
	Tunnellink LogEventType = iota
	HTTP
	TCP
	UDP
)
...
// the the first two are limited to failure conditions that lead to tunnellink shutting down.
type LogLevel int8
