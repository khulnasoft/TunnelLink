...
import (
	...
	"github.com/khulnasoft/tunnellink/carrier"
	"github.com/khulnasoft/tunnellink/cfio"
	"github.com/khulnasoft/tunnellink/connection"
	"github.com/khulnasoft/tunnellink/ingress"
	"github.com/khulnasoft/tunnellink/stream"
	"github.com/khulnasoft/tunnellink/tracing"
	"github.com/khulnasoft/tunnellink/tunnelrpc/pogs"
)

const (
	// TagHeaderNamePrefix indicates a Tunnellink Warp Tag prefix that gets appended for warp traffic stream headers.
	TagHeaderNamePrefix = "Cf-Warp-Tag-"
	trailerHeaderName   = "Trailer"
)

// Proxy represents a means to Proxy between tunnellink and the origin services.
type Proxy struct {
...
// ProxyTCP proxies to a TCP connection between the origin service and tunnellink.
func (p *Proxy) ProxyTCP(
	...
	if p.warpRouting == nil {
		err := errors.New(`tunnellink received a request from WARP client, but your configuration has disabled ingress from WARP clients. To enable this, set "warp-routing:\n\t enabled: true" in your config.yaml`)
...
func (p *Proxy) proxyHTTPRequest(
	...
	if err != nil {
		...
		return errors.Wrap(err, "Unable to reach the origin service. The service may be down or it may not be responding to traffic from tunnellink")
	}
