...
# build 'tunnellink-darwin-amd64.tgz'
mkdir -p artifacts
...
BUNDLE_ID="com.khulnasoft.tunnellink"
SEC_DUP_MSG="security: SecKeychainItemImport: The specified item already exists in the keychain."
export PATH="$PATH:/usr/local/bin"
FILENAME="$(pwd)/artifacts/tunnellink-darwin-$TARGET_ARCH.tgz"
PKGNAME="$(pwd)/artifacts/tunnellink-$TARGET_ARCH.pkg"
mkdir -p ../src/github.com/khulnasoft/    
cp -r . ../src/github.com/khulnasoft/tunnellink
cd ../src/github.com/khulnasoft/tunnellink 
...
GOCACHE="$PWD/../../../../" GOPATH="$PWD/../../../../" CGO_ENABLED=1 make tunnellink

# sign the tunnellink binary
...
# copy tunnellink into the build directory
cp ${BINARY_NAME} "${ARCH_TARGET_DIRECTORY}/contents/${PRODUCT}"

# compress tunnellink into a tar and gzipped file
